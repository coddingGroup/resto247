  ...
  let funMenu = (products  = productToDisplay, reset=false) =>{
    ...
    if(products.length >=  lastIndex){
      ...
      let menu = opElement.map((product) => {
        return (
            <div className="">
              <RenderCard3 key={product.id +"op"} cart={props.cart} addToCart={props.addToCart} removeToCart={props.removeToCart}  item={product} />
            </div>
        );
      });
      if(products.length === lastIndex){
            ...
            <Col sm="4">
              <Card className="mt-2">
                <CardTitle className="mt-1 col">
                  <h3>  Products on Cart  <span class="float-right "><AddedCarts cart={props.cart} /> </span>
                      <span class="fa fa-print mt-2 float-right"> </span>
                  </h3>
                </CardTitle>
                <CardBody> <TableAddedCarts removeToCart={props.removeToCart} cart={props.cart} /> </CardBody>
              </Card>
